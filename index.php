...
function awebon_plagiarism_check_plagiarism( $data , $postarr ) {
	...
	if(!in_array($data['post_type'],get_option('awebon_plagiarism_post_type_to_check')))
		return $data;
	...
	for ($i=0; $i < $chunks_count; $i++) {
		if(session_status() != PHP_SESSION_ACTIVE)
			session_start();
		$response = wp_remote_get( "https://www.googleapis.com/customsearch/v1?key=".$awebon_google_api_key."&cx=".$awebon_google_search_engine_id."&q=".urlencode($chunks[$i])."&fields=queries(request/totalResults)&exactTerms=".urlencode($chunks[$i]));
...
function awebon_plagiarism_save_plagiarism_search_results( $post_id ) {
	...
	if(!in_array(get_post()->post_type,get_option('awebon_plagiarism_post_type_to_check')))
		return;
...
function awebon_plagiarism_add_plagiarism_meta_box() {
	add_meta_box('awebon_plagiarism_meta_box_id','Awebon Plagiarism Results','awebon_plagiarism_show_plagiarism_meta_box', get_option('awebon_plagiarism_post_type_to_check'), 'advanced', 'high');
}
...
function awebon_plagarism_settings_options(){ 
	...
	if(isset($_POST['awebon_plagiarism_ispost']) && $_POST['awebon_plagiarism_ispost'] == 'true' && current_user_can('manage_options')) {
	  ...
	  if (isset($_POST['awebon_plagiarism_post_type_to_check'])){
	    $awebon_plagiarism_post_type_to_check = $_POST['awebon_plagiarism_post_type_to_check'];
	    update_option('awebon_plagiarism_post_type_to_check', $awebon_plagiarism_post_type_to_check);
	  }		    
	}else{
	  $awebon_google_search_engine_id = get_option('awebon_google_search_engine_id');
	  $awebon_google_api_key = get_option('awebon_google_api_key');
	  ...
	  $awebon_plagiarism_post_type_to_check = get_option('awebon_plagiarism_post_type_to_check');
	}
		$post_types = array_merge(['post','page'],get_post_types(['public'=>true,'_builtin' => false],'names'));
		$awebon_plagiarism_post_type_to_checkbox='';
		foreach ( $post_types as $post_type ){		 
		   $awebon_plagiarism_post_type_to_checkbox = $awebon_plagiarism_post_type_to_checkbox.'<input type="checkbox" name="awebon_plagiarism_post_type_to_check[]" value="'.$post_type.'" style="margin-top:5px;margin-bottom:5px"'.((in_array($post_type, $awebon_plagiarism_post_type_to_check))?'checked=checked':'').' >'.$post_type.'</input>&nbsp;';
		}
	?>
		    ...
		    <?php wp_nonce_field('awebon-plagiarism-settings'); 
				...
				__('Post types for Plagiarism Check') => $awebon_plagiarism_post_type_to_checkbox,
				__('API Key') => '<input type="text" name="awebon_google_api_key" value="'.esc_html($awebon_google_api_key).'" size="75">',				
...
function awebon_plagarism_settings_action(){
	...
	$awebon_plagiarism_post_type_to_check = get_option('awebon_plagiarism_post_type_to_check');
	if (empty($awebon_plagiarism_post_type_to_check)){
		update_option('awebon_plagiarism_post_type_to_check',['post']);
	}
